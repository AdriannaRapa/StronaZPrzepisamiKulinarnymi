...
document.querySelector("form").addEventListener("submit", async function (event) {
    ...
    console.log("Dane do wysłania:", { name, category, ingredients, steps }); // Sprawdź dane w konsoli
    ...
    try {
      ...
      const response = await fetch("/recipes", {
        ...
        body: JSON.stringify({ name, category, ingredients, steps }),
      });
  
      const result = await response.json();
      console.log("Odpowiedź serwera:", result); // Sprawdź odpowiedź w konsoli
      if (response.ok) {
        alert(result.message);
        window.location.href = "/recipes.html";
      } else {
        alert(result.error || "Nie udało się dodać przepisu.");
      }
    } catch (error) {
